...
public class AIController {
    ...
    public ResponseEntity<AIResponse> recommend(@AuthenticationPrincipal UserDetails principal,
        ...
        String prompt = buildPrompt(user, requestOverride != null ? requestOverride.getPrompt() : null);
    ...
    private String formatMeal(Meal meal) {
        String items = meal.getItems().stream()
                ...
                .collect(Collectors.joining(", "));
        return String.format("%s on %s (%.1f kcal): %s", meal.getName(), meal.getMealDate(), meal.getTotalCalories(), items);
    }
